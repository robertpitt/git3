...
type ObjectStore interface {
	...
	OpenRange(context.Context, string, int64, int64) (store.Range, error)
...
func verifyRemote(ctx context.Context, storage ObjectStore, key string, size int64, oid string, chunk int64) error {
	...
	for start := int64(0); start < size; {
		...
		if _, err := copyRange(ctx, storage, key, start, end, size, hash); err != nil {
			return err
		}
		start = end + 1
	}
...
func copyRange(ctx context.Context, storage ObjectStore, key string, start, end, total int64, dst io.Writer) (int64, error) {
	part, err := storage.OpenRange(ctx, key, start, end)
	if err != nil {
		return 0, err
	}
	want := end - start + 1
	if part.Size != want || part.TotalSize != total {
		_ = part.Body.Close()
		return 0, fmt.Errorf("invalid LFS range response")
	}
	...
	if n != want {
		return n, fmt.Errorf("short or oversized LFS range response")
	}
	return n, nil
}
...
func (a *Agent) downloadRanges(ctx context.Context, backend Backend, oid, key string, size int64, file *os.File) error {
	...
	for range workers {
		...
		go func() {
			defer group.Done()
			for span := range jobs {
				writer := io.NewOffsetWriter(file, span.start)
				n, err := copyRange(ctx, backend.Store, key, span.start, span.end, size, writer)
				if err == nil {
					progressMu.Lock()
					completed += n
					err = a.progress(oid, completed, int(n))
					progressMu.Unlock()
				}
				if err != nil {
					select {
